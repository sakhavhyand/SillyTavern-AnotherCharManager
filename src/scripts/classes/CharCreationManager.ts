...
export class CharCreationManager {
     ...
     */
    static FIELD_CONFIGURATIONS: Record<string, string> = {
    ...
    /**
     * Initializes event listeners for the character creation interface.
     */
    initializeCharacterCreationEvents(): void {
        ...
        // Add the avatar
        $('#acm_add_avatar_button').on('change', (event: JQuery.TriggeredEvent) => {
            this.loadAvatar(event.currentTarget as HTMLInputElement);
        });
    }
     ...
     */
    toggleCharacterCreationPopup(): void {
    ...
    /**
     * Updates the layout of the character creation interface.
     */
    updateLayout(showAdvanced: boolean): void {
        if (!showAdvanced) {
     ...
     */
    async loadAvatar(input: HTMLInputElement): Promise<void> {
     ...
     */
    async initiateCharacterCreation(): Promise<void> {
