...
export function getPreset(presetIndex) {
    if (presetIndex < 0 || presetIndex >= extensionSettings.acm.dropdownPresets.length) {
        throw new Error('Invalid preset index');
    }
    return {...extensionSettings.acm.dropdownPresets[presetIndex]};
}
...
export function updatePresetName(presetIndex, name) {
    ...
    const updatedPresets = [...extensionSettings.acm.dropdownPresets];
